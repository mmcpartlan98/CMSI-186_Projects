...
void setup()
{
  ...
  Serial.println(F("          (deg)      (deg)       Age                      Age  (f)    --- from GPS ----  ---- to 300 HMB ----  RX    RX        Fail"));
  Serial.println(F("---------------------------------------------------------------------------------------------------------------------------------------"));
  lcd.begin(16, 2);
}
...
void loop()
{
  static const double HOME_LAT = 37.464433, HOME_LON = -122.440399;
  ...
  float distanceMiToHome =
    (float)TinyGPSPlus::distanceBetween(
      gps.location.lat(),
      gps.location.lng(),
      HOME_LAT, 
      HOME_LON) * 0.000621371;
  printFloat(distanceMiToHome, gps.location.isValid(), 7, 4);
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Distance to HMB:");
  lcd.setCursor(0, 2);
  lcd.print(distanceMiToHome, 4);
  lcd.print(" miles");
